...
class Bot(BotBase):
    ...
    def update_db(self):
        ...
        for member in guildneeded.members if not member.bot: db.multiexec("INSERT OR IGNORE INTO exp (UserID) VALUES (?)", (member.id,))
        ...
        for id_ in to_remove: db.multiexec("DELETE FROM exp WHERE UserID = ?", (id_,))
